...
class Model:
    ...
    def _build_loss(self, reg_lambda: float):
        with tf.variable_scope('Loss'):
            ...
            # l2_reg_attn = tf.add_n([
            #     tf.nn.l2_loss(weight) for weight in self.reg_vas_attn])

            self.loss = cross_entropy_mean + reg_lambda * l2_reg 
            #tf.summary.scalar('l2_reg', l2_reg)
            tf.summary.scalar('loss', self.loss)
    def _return_feature(self,idx):
        inputs = {
            self.idx: idx,

            self.isTrain: False}

        return self.sess.run(tf.gather(self.logits, self.idx, axis=0), feed_dict=inputs)
